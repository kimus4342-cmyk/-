...
def _chat(system: str, user: str, model: str = WRITING_MODEL, max_tokens: int = WRITING_MAX_TOKENS) -> str:
    if model.startswith("claude"):
        ...
        resp = client.messages.create(
            ...
            # 시스템 프롬프트(WRITING_SYSTEM_PROMPT, ~17,000자)를 캐싱 — 한 파이프라인 실행 안에서
            # 초안·조건부 교정(글자수 보강, 인용 오류 수정 등)·고도화 압축 보정까지 같은 프롬프트가
            # 반복 호출되므로, 첫 호출 이후로는 캐시 적중 시 해당 부분 입력 비용이 1/10로 줄어든다.
            system=[
                {
                    "type": "text",
                    "text": system,
        ...
        # extended thinking을 쓰는 모델은 content[0]이 ThinkingBlock일 수 있어
        # 텍스트 블록만 골라 이어붙인다.
...
def run_writing_agent(research: ResearchOutput, review_insights: ReviewInsights | None = None) -> str:
    ...
    questions_block = (
        f"\n[독자가 검색 전 궁금해할 질문 — 섹션 순서 설계에 참고. 앞 질문에 답하면 다음 질문으로 자연스럽게 이어지도록 섹션을 배치할 것]:\n{research.reader_questions}"
        if research.reader_questions else ""
    )
